...
def _parse_image_function(example_proto):
   # Parse the input tf.Example proto using the dictionary above.
    ...
    image_png=tf.image.resize(image_png,[512,768])
...
def encoderGC_model(C=BottleNeck):
    ...
    input=layers.Input(shape=(512,768,3))
...
def generator_model(C=BottleNeck):
    ...
    input=layers.Input(shape=(32,48,C))
...
def multiscale_discriminator_model():
    ...
    x=layers.Input(shape=(512,768,3))
...
def save_img(model,image,epoch,index):
    ...
    for i in range(gen_imgs.shape[0]):
        ...
        with tf.io.gfile.GFile('../Semcom/GAN/imgs_0424/C{:02d}_image_{:02d}_at_epoch_{:04d}_kodak.png'.format(BottleNeck,index,epoch), 'wb') as file:
...
model_dir='../Semcom/GAN/imgs_0424'

input_img=layers.Input(shape=(512,768,3))
x=input_img
for i in range(len(encoderGC.layers)):
    if not isinstance(encoderGC.layers[i],layers.InputLayer):
      x=encoderGC.layers[i](x)
w_hat=layers.Lambda(quantizier_aftertrain,output_shape=(32, 48, 8))(x)
