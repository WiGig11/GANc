...
class AnalysisTransform(tf.keras.Sequential):
    ...
    def __init__(self,Bottleneck):
        ...
        self.add(tf.keras.layers.Conv2D(
            Bottleneck, kernel_size = (3, 3), strides = (1,1),name="layer_5",
            padding="valid",
...
class BMSHJ2018Model(tf.keras.Model):
  ...
  def call(self, x, training):
    ...
    entropy_model = LocationScaleIndexedEntropyModel(
        tfc.NoisyNormal, self.num_scales, self.scale_fn, coding_rank=3,
        compression=False)
    side_entropy_model = ContinuousBatchedEntropyModelchz(
