...
from utils import ContinuousIndexedEntropyModelwithSoftQuant,LocationScaleIndexedEntropySoftModel
...
class AnalysisTransform(tf.keras.Sequential):
    ...
    def __init__(self):
        ...
        self.add(tf.keras.layers.Conv2D(
            60, kernel_size = (7, 7), strides = (1,1),name="layer_0",
            padding="valid",
            activation=tfc.GDN(name="gdn_0")))
        self.add(tf.keras.layers.ReLU())

        self.add(tf.keras.layers.Conv2D(
            120, kernel_size = (3, 3), strides = (2,2),name="layer_1",
            padding="same",
            activation=tfc.GDN(name="gdn_1")))
        self.add(tf.keras.layers.ReLU())

        self.add(tf.keras.layers.Conv2D(
            240, kernel_size = (3, 3), strides = (2,2),name="layer_2",
            padding="same",
            activation=tfc.GDN(name="gdn_2")))
        self.add(tf.keras.layers.ReLU())

        self.add(tf.keras.layers.Conv2D(
            480, kernel_size = (3, 3), strides = (2,2),name="layer_3",
            padding="same",
            activation=tfc.GDN(name="gdn_3")))
        self.add(tf.keras.layers.ReLU())

        self.add(tf.keras.layers.Conv2D(
            960, kernel_size = (3, 3), strides = (2,2),name="layer_4",
            padding="same",
            activation=tfc.GDN(name="gdn_4")))
        self.add(tf.keras.layers.ReLU())

        self.add(tf.keras.layers.Lambda(mypad,arguments={'size':1}))

        self.add(tf.keras.layers.Conv2D(
            120, kernel_size = (3, 3), strides = (1,1),name="layer_5",
            padding="valid",
...
class MainLine(tf.keras.Sequential):
    def __init__(self,idx,padsize,numfilters1,kernelsize1,stride1,se_factor,numfilters2,kernelsize2,stride2):
        super().__init__(name="res block main line ")
...
class SynthesisTransform2(tf.keras.Sequential):
  """The synthesis transform."""
...
class AnalysisTransform2(tf.keras.Sequential):
  """The analysis transform."""
...
class BMSHJ2018Model(tf.keras.Model):
  ...
  def __init__(self, lmbda, num_filters, num_scales, scale_min, scale_max):
    ...
    self.analysis_transform = AnalysisTransform2(num_filters)
    self.synthesis_transform = SynthesisTransform2(num_filters)
  ...
  def call(self, x, training):
    ...
    entropy_model = LocationScaleIndexedEntropySoftModel(
        tfc.NoisyNormal, self.num_scales, self.scale_fn, coding_rank=3,
        compression=False)
    side_entropy_model = ContinuousIndexedEntropyModelwithSoftQuant(
...
def parse_args(argv):
  ...
  parser.add_argument(
      "--model_path", default="/chz/bmshj2018",
